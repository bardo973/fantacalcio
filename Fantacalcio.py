...
<style>
    ...
    .stApp {
        background:
            radial-gradient(1100px 600px at 12% -10%, rgba(0,210,106,0.13), transparent 60%),
            radial-gradient(900px 500px at 95% 0%, rgba(78,168,255,0.12), transparent 55%),
            linear-gradient(180deg, #080b14 0%, #0e1226 55%, #0a0d1c 100%);
    }
    ...
    /* ---------- Sidebar ---------- */
    .stSidebar {
        background: linear-gradient(180deg, #0d1024 0%, #0a0c1c 100%) !important;
    ...
    button[data-baseweb="tab"][aria-selected="true"] {
        background: linear-gradient(90deg, rgba(0,210,106,0.22), rgba(0,168,84,0.18)) !important;
        color: #eafff3 !important;
        box-shadow: inset 0 0 0 1px rgba(0,210,106,0.35);
    }
    div[data-baseweb="tab-highlight"] { background: transparent !important; }

    /* ---------- Metriche, expander, input ---------- */
    ...
    div[data-testid="stMetricLabel"] { color: #93a3ba !important; }
    ...
    ::-webkit-scrollbar-thumb:hover { background: rgba(0,210,106,0.6); }
    ...
    .op-badge {
        font-size: 0.72em; font-weight: 800; letter-spacing: .6px;
        padding: 2px 10px; border-radius: 999px; text-transform: uppercase;
    }
    .op-data { font-size: 0.78em; color: #8fa0b8; }
    .op-ultima {
        font-size: 0.66em; font-weight: 800; letter-spacing: .8px;
        padding: 2px 8px; border-radius: 999px;
        background: rgba(255,255,255,0.10); color: #dfe7f2;
    }
    .op-dett { margin-top: 5px; color: #e7edf6; font-size: 0.94em; line-height: 1.35; }

    /* ---------- Watchlist chip ---------- */
    .wl-chip {
        background: linear-gradient(140deg, rgba(255,215,0,0.14), rgba(255,255,255,0.03));
...
# ============================================================
# 🤖 SMART SCOUT — Ricerca Intelligente Titolari Low Cost
# ============================================================

def smart_scout_titolari_lowcost(df, ruoli_richiesti, max_prezzo, min_titolarita, solo_svincolati=True):
    """
    Trova giocatori titolari a basso costo tra Difensori, Centrocampisti e Attaccanti.
    """
    df_f = df.copy()
    df_f = df_f[df_f["Ruolo"].isin(ruoli_richiesti)]
    df_f = df_f[df_f["Quotazione"] <= max_prezzo]
    if "Indice_Titolarita" in df_f.columns:
        df_f = df_f[df_f["Indice_Titolarita"] >= min_titolarita]
    if solo_svincolati and "Proprietario" in df_f.columns:
        df_f = df_f[df_f["Proprietario"].str.contains("Svincolato", na=False)]
    sort_cols = []
    if "Indice_Titolarita" in df_f.columns:
        sort_cols.append("Indice_Titolarita")
    if "Indice_Affare" in df_f.columns:
        sort_cols.append("Indice_Affare")
    if "FantaMedia" in df_f.columns:
        sort_cols.append("FantaMedia")
    if sort_cols:
        df_f = df_f.sort_values(sort_cols, ascending=[False]*len(sort_cols))
    return df_f


def render_smart_scout_card(row, stats_per_stagione=None, stats_2627=None):
    """Card compatta per Smart Scout."""
    ...
    return f'''<div class="{smart_class}" style="background:linear-gradient(135deg, rgba(30,30,63,0.95) 0%, rgba(42,42,74,0.8) 100%);backdrop-filter:blur(10px);border-radius:12px;padding:12px;margin-bottom:8px;border-left:4px solid {colore};box-shadow:0 8px 32px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.1);display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
        ...
        <div style="text-align:center;min-width:60px;">
            <div style="font-size:1.5em;font-weight:bold;color:#ffd700;">{fm}</div>
            <div style="font-size:0.7em;color:#888;">FM</div>
        </div>
        <div style="text-align:center;min-width:50px;">
            <div style="font-size:1.3em;font-weight:bold;color:#00d26a;">{quot}</div>
            <div style="font-size:0.7em;color:#888;">cr</div>
        </div>
        <div style="text-align:center;min-width:50px;">
            <div style="font-size:1.1em;font-weight:bold;color:#3b82f6;">{aff:.2f}</div>
            <div style="font-size:0.7em;color:#888;">IA</div>
        </div>
        <div style="min-width:55px;">{gauge_tit}</div>
        {flame}
    </div>'''
...
# ============================================================
# REGISTRO OPERAZIONI — annullamento singolo
# ============================================================
import copy as _copy


def _snap_stato() -> dict:
    """Fotografia dello stato di gioco (squadre, contratti, prestiti)."""
    return {
        "squadre": _copy.deepcopy(st.session_state.get("squadre", {})),
        "contratti": _copy.deepcopy(st.session_state.get("contratti", {})),
        "prestiti": _copy.deepcopy(st.session_state.get("prestiti", [])),
    }


# Ad ogni rerun salviamo lo stato "prima" delle azioni di questo giro.
st.session_state._stato_inizio_run = _snap_stato()


def registra_op(operazione: str, dettagli: str, stato_prima: dict = None):
    """Registra un'operazione di mercato salvando lo stato precedente."""
    st.session_state.storico_mercato.insert(0, {
        "ID": f"{datetime.now().strftime('%Y%m%d%H%M%S%f')}-{random.randint(1000, 9999)}",
        "Data": datetime.now().strftime("%Y-%m-%d %H:%M"),
        "Operazione": operazione,
        "Dettagli": dettagli,
        "_stato_prima": stato_prima or st.session_state.get("_stato_inizio_run") or _snap_stato(),
    })


def storico_pubblico() -> list:
    """Storico senza campi tecnici (per tabelle ed export)."""
    return [
        {k: v for k, v in op.items() if not k.startswith("_") and k != "ID"}
        for op in st.session_state.get("storico_mercato", [])
    ]


def annulla_operazione(idx: int):
    """Annulla una singola operazione ripristinando lo stato precedente."""
    storico = st.session_state.get("storico_mercato", [])
    if idx < 0 or idx >= len(storico):
        return False, "Operazione non trovata."
    op = storico[idx]
    snap = op.get("_stato_prima")
    if not snap:
        return False, "Operazione registrata prima dell'aggiornamento: non è annullabile."
    st.session_state.squadre = _copy.deepcopy(snap.get("squadre", {}))
    st.session_state.contratti = _copy.deepcopy(snap.get("contratti", {}))
    st.session_state.prestiti = _copy.deepcopy(snap.get("prestiti", []))
    successive = len(storico[:idx])
    st.session_state.storico_mercato = storico[idx + 1:]
    st.session_state._stato_inizio_run = _snap_stato()
    invalidate_cache()
    save_state()
    msg = f"Annullata: {op.get('Operazione')} — {op.get('Dettagli')}"
    if successive:
        msg += f" (insieme a {successive} operazioni successive)"
    return True, msg


def rimuovi_da_watchlist(nome: str):
    """Rimuove un singolo giocatore dalla watchlist."""
    if nome in st.session_state.get("watchlist", []):
        st.session_state.watchlist = [w for w in st.session_state.watchlist if w != nome]
        save_state()
        return True
    return False
...
if menu == "🏠 Dashboard":
    ...
    if st.session_state.storico_mercato:
        ...
        hist = pd.DataFrame(storico_pubblico())
    ...
    if st.session_state.storico_mercato:
        ...
        storico_ord = sorted(storico_pubblico(), key=lambda x: x.get("Data", ""))
...
if menu == "🔍 Scouting & Database":
    ...
    if df.empty:
        ...
        # ============================================================
        # 🤖 SMART SCOUT — Titolari a Basso Costo
        # ============================================================
        with st.expander("🤖 Smart Scout — Titolari Low Cost", expanded=False):
            st.subheader("🎯 Trova Titolari a Basso Costo")
            st.caption("Cerca automaticamente giocatori titolari a basso costo tra Difensori, Centrocampisti e Attaccanti.")

            ss_col1, ss_col2, ss_col3, ss_col4 = st.columns(4)
            with ss_col1:
                ss_ruoli = st.multiselect(
                    "Ruoli",
                    ["D", "C", "A"],
                    default=["D", "C", "A"],
                    format_func=lambda x: {"D": "🛡️ Difensori", "C": "⚙️ Centrocampisti", "A": "⚔️ Attaccanti"}[x],
                    key="smart_ruoli"
                )
            with ss_col2:
                ss_max_prezzo = st.slider("Max Quotazione (cr)", 1, 50, 25, key="smart_prezzo")
            with ss_col3:
                ss_min_tit = st.slider("Min Titolarità", 0, 100, 60, key="smart_tit")
            with ss_col4:
                ss_solo_svinc = st.checkbox("Solo Svincolati", value=True, key="smart_svinc")
                ss_top_n = st.number_input("Mostra top", 1, 20, 5, key="smart_n")

            if st.button("🔍 Cerca Titolari Low Cost", type="primary", use_container_width=True, key="smart_btn"):
                df_smart = smart_scout_titolari_lowcost(
                    df_f if 'df_f' in locals() else df,
                    ss_ruoli,
                    ss_max_prezzo,
                    ss_min_tit,
                    ss_solo_svinc
                ...
                if not df_smart.empty:
                    st.success(f"✅ Trovati {len(df_smart)} giocatori titolari a basso costo!")

                    for ruolo in ss_ruoli:
                        df_r = df_smart[df_smart["Ruolo"] == ruolo].head(ss_top_n)
                        if not df_r.empty:
                            ruolo_nome = {"D": "🛡️ Difensori", "C": "⚙️ Centrocampisti", "A": "⚔️ Attaccanti"}[ruolo]
                            st.markdown(f"**{ruolo_nome}** — {len(df_r)} trovati")
                            stats_ps_smart = st.session_state.get("stats_per_stagione", {})
                            stats_2627_smart = stats_ps_smart.get("2026-27") if "2026-27" in stats_ps_smart else None
                            for _, row in df_r.iterrows():
                                st.markdown(render_smart_scout_card(row.to_dict(), stats_ps_smart, stats_2627_smart), unsafe_allow_html=True)
                else:
                    st.warning("⚠️ Nessun giocatore trovato con questi criteri.")
                    st.info("💡 Suggerimento: prova a ridurre la titolarità minima o aumentare il prezzo massimo.")
...
# ============================================================
        ...
        with st.expander("⭐ Watchlist", expanded=True):
            ...
            if st.session_state.watchlist:
                st.markdown("**🎯 Giocatori seguiti** — clicca ✖️ per rimuoverli uno alla volta")
                cols_rm = st.columns(4)
                for i_wl, nome_wl in enumerate(list(st.session_state.watchlist)):
                    with cols_rm[i_wl % 4]:
                        cn, cx = st.columns([5, 1])
                        with cn:
                            st.markdown(f"<div class='wl-chip'>⭐ {nome_wl}</div>", unsafe_allow_html=True)
                        with cx:
                            if st.button("✖️", key=f"rm_wl_{i_wl}_{nome_wl}", help=f"Rimuovi {nome_wl} dalla watchlist"):
                                rimuovi_da_watchlist(nome_wl)
                                st.toast(f"{nome_wl} rimosso dalla watchlist", icon="🗑️")
                                st.rerun()
                st.markdown("---")
                ...
                if st.session_state.get("_asta_vincitore") and g_asta:
                    ...
                    if st.button("✅ Conferma Assegnazione", type="primary", use_container_width=True):
                        if st.session_state.squadre[vincitore]["crediti"] >= prezzo_vincita:
                            ...
                            registra_op("ASTA", f"{vincitore} aggiudica {g_asta} ({info['Ruolo']}) per {prezzo_vincita}cr — Contratto fino al {scad_acq}")
...
if menu == "🛒 Mercato":
            ...
            if len(svinc) > 0:
                ...
                if st.button("Conferma Acquisto", type="primary"):
                    if cred >= prezzo:
                        ...
                        registra_op("ACQUISTO", f"{sq} acquista {g_sel} ({info['Ruolo']}) per {prezzo}cr — Contratto fino al {scad_acq}")
                        invalidate_cache()
                        save_state()
    ...
    with t_vend:
        ...
        if rosa_proprieta_list:
            ...
            if st.button("Conferma Vendita"):
                ...
                registra_op("SVINCOLO", f"{sq_v} svincola {g_v}, incassa {prezzo_v}cr")
                invalidate_cache()
                save_state()
        ...
        if rosa_rinnovabili:
            ...
            if crediti_disp < costo_rinnovo:
                ...
                if st.button("📝 Conferma Rinnovo", type="primary"):
                    ...
                    registra_op("RINNOVO", f"{sq_r} rinnova {g_r} fino al {nuovo_mese:02d}/{nuova_scadenza} per {costo_rinnovo}cr (quotazione listone)")
                    invalidate_cache()
                    save_state()
    ...
    with t_reg:
        ...
        if storico:
            st.caption(
                f"**{len(storico)}** operazioni registrate. "
                "Puoi annullarle una alla volta: l'operazione più recente è in cima."
            )

            f1, f2 = st.columns([2, 3])
            tipi = sorted({op.get("Operazione", "—") for op in storico})
            with f1:
                filtro_tipo = st.multiselect("Filtra per tipo", tipi, key="reg_filtro_tipo")
            with f2:
                cerca = st.text_input("Cerca nel registro", key="reg_cerca", placeholder="Nome giocatore o squadra…")

            COLORI_OP = {
                "ACQUISTO": "#00d26a", "ASTA": "#ffd700", "SVINCOLO": "#ff6b6b",
                "RINNOVO": "#4ea8ff", "FINE PRESTITO": "#b58cff",
            }

            for i, op in enumerate(storico):
                tipo_op = op.get("Operazione", "—")
                dett = op.get("Dettagli", "")
                if filtro_tipo and tipo_op not in filtro_tipo:
                    continue
                if cerca and cerca.lower() not in f"{tipo_op} {dett}".lower():
                    continue
                colore = COLORI_OP.get(tipo_op, "#8aa0c0")
                tag_ultima = "<span class='op-ultima'>ULTIMA</span>" if i == 0 else ""
                data_op = op.get("Data", "")
                c_card, c_btn = st.columns([6, 1])
                with c_card:
                    st.markdown(
                        f"<div class='op-card' style='border-left-color:{colore};'>"
                        f"<div class='op-head'><span class='op-badge' style='background:{colore}1a;color:{colore};"
                        f"border:1px solid {colore}55;'>{tipo_op}</span>"
                        f"<span class='op-data'>🕒 {data_op}</span>"
                        f"{tag_ultima}</div>"
                        f"<div class='op-dett'>{dett}</div></div>",
                        unsafe_allow_html=True,
                    )
                with c_btn:
                    key_op = op.get("ID", f"idx{i}")
                    if st.session_state.get("_conferma_annulla") == key_op:
                        if st.button("✔️ Sì", key=f"ok_ann_{key_op}", use_container_width=True):
                            ok, msg = annulla_operazione(i)
                            st.session_state.pop("_conferma_annulla", None)
                            if ok:
                                st.toast(msg, icon="↩️")
                            else:
                                st.error(msg)
                            st.rerun()
                        if st.button("✖️ No", key=f"no_ann_{key_op}", use_container_width=True):
                            st.session_state.pop("_conferma_annulla", None)
                            st.rerun()
                    else:
                        disabled = not op.get("_stato_prima")
                        if st.button(
                            "↩️ Annulla", key=f"ann_{key_op}", use_container_width=True, disabled=disabled,
                            help=("Operazione precedente all'aggiornamento: non annullabile"
                                  if disabled else
                                  "Ripristina la situazione precedente a questa operazione"),
                        ):
                            st.session_state._conferma_annulla = key_op
                            st.rerun()

                if st.session_state.get("_conferma_annulla") == op.get("ID", f"idx{i}") and i > 0:
                    st.warning(
                        f"⚠️ Questa non è l'ultima operazione: annullandola verranno annullate anche "
                        f"le **{i}** operazioni successive (crediti e rose tornano com'erano prima)."
                    )

            st.markdown("---")
            with st.expander("📋 Vista tabella & pulizia registro"):
                st.dataframe(pd.DataFrame(storico_pubblico()), use_container_width=True, hide_index=True)
                if st.button("🗑️ Svuota registro (non modifica rose e crediti)"):
                    st.session_state.storico_mercato = []
                    save_state()
                    st.rerun()
        else:
...
if menu == "🤝 Scambi & Prestiti":
    ...
    if st.button("Finalizza", type="primary"):
        if not g1 and not g2 and d1 == 0 and d2 == 0:
            ...
            registra_op(tipo.upper(), msg + (f" | Conguaglio: {d1}cr vs {d2}cr" if d1 or d2 else ""))
            invalidate_cache()
            save_state()
            st.rerun()
    ...
    if st.session_state.prestiti:
        ...
        if st.button("Termina prestito e riporta in rosa originale"):
            ...
            if to_remove is not None:
                ...
                registra_op("FINE PRESTITO", f"{gp} torna a {da_sq}")
                invalidate_cache()
                save_state()
                        ...
                        if g_rinn:
                            ...
                            if st.session_state.squadre[sq]["crediti"] < quot_rinn:
                                ...
                                if st.button(f"📝 Rinnova {g_rinn}", key=f"btn_rinn_{sq}_{g_rinn}"):
                                    ...
                                    registra_op("RINNOVO", f"{sq} rinnova {g_rinn} fino al {nuovo_mese_rinn:02d}/{nuovo_anno_rinn} per {quot_rinn}cr")
                                    invalidate_cache()
                                    save_state()
    ...
    with tabs[1]:
        if not st.session_state.stats_storiche.empty:
            ...
            if len(giocatori_stats) > 0:
                ...
                # 📊 GRAFICI STATISTICHE
                # ============================================================
                if not df_g.empty and len(df_g) > 1:
                    st.markdown("---")
                    st.subheader("📊 Grafici Andamento")

                    if "Gol" in df_g.columns and df_g["Gol"].notna().any():
                        fig_gol = go.Figure(data=[
                            go.Bar(
                                x=df_g["Stagione"],
                                y=df_g["Gol"],
                                marker_color="#00d26a",
                    ...
                    if "FantaMedia" in df_g.columns and df_g["FantaMedia"].notna().any():
                        fig_fm = go.Figure()
                        fig_fm.add_trace(go.Scatter(
                            ...
                            text=[f"{v:.2f}" for v in df_g["FantaMedia"]],
                            textposition="top center",
                            name="FantaMedia"
                        ))
                        fig_fm.update_layout(
                            title="📈 Andamento FantaMedia",
                            paper_bgcolor="rgba(0,0,0,0)",
                            plot_bgcolor="rgba(0,0,0,0)",
                            font_color="#ddd",
                            xaxis_title="Stagione",
                    ...
                    has_gol = "Gol" in df_g.columns and df_g["Gol"].notna().any()
                    has_ast = "Assist" in df_g.columns and df_g["Assist"].notna().any()
                    if has_gol or has_ast:
                        fig_combo = go.Figure()
                        if has_gol:
                        ...
                        fig_combo.update_layout(
                            title="⚽🅰️ Gol & Assist per Stagione",
                            barmode="group",
                            paper_bgcolor="rgba(0,0,0,0)",
                            plot_bgcolor="rgba(0,0,0,0)",
                            font_color="#ddd",
                    ...
                    if "Partite" in df_g.columns and df_g["Partite"].notna().any():
                        ...
                        fig_pres.add_hline(y=38, line_dash="dash", line_color="#888", annotation_text="Max 38")
                        fig_pres.update_layout(
                            title="🏃 Presenze per Stagione",
                            paper_bgcolor="rgba(0,0,0,0)",
                            plot_bgcolor="rgba(0,0,0,0)",
                            font_color="#ddd",
                            xaxis_title="Stagione",
                    ...
                    if len(df_g) >= 2:
                        st.markdown("#### 🕸️ Profilo Completo")
                        radar_metrics = []
                        radar_values = []
                        for col in ["FantaMedia", "Gol", "Assist", "Partite"]:
                            if col in df_g.columns and df_g[col].notna().any():
                                val = df_g[col].mean()
                        ...
                        if len(radar_metrics) >= 3:
                            ...
                            fig_radar.update_layout(
                                polar=dict(
                                    radialaxis=dict(visible=True, range=[0, max(radar_values)*1.2]),
                                    bgcolor="rgba(0,0,0,0)"
                                ),
                                paper_bgcolor="rgba(0,0,0,0)",
                                font_color="#ddd",
                                showlegend=False,
                                title=f"🕸️ Profilo Medio — {g_sel}"
                            )
...
if menu == "⚙️ Importa & Esporta":
    ...
    with tab_exp:
        ...
        with pd.ExcelWriter(buffer_exp, engine="openpyxl") as writer:
            ...
            if st.session_state.storico_mercato:
                pd.DataFrame(storico_pubblico()).to_excel(writer, index=False, sheet_name="Storico")
        st.download_button(
                ...
                for ruolo in ["P", "D", "C", "A"]:
                    ...
                    for _, row in valid.iterrows():
                        ...
                        if st.session_state.squadre[sq_sim]["crediti"] >= prezzo:
                            ...
                            registra_op("SIMULATORE → ROSA", f"{sq_sim} acquista {nome_g} ({ruolo_db}) per {prezzo}cr — da Simulatore Rosa")
                            applicati += 1
                        else:
